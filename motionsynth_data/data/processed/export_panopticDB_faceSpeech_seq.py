...
def process_file_withSpeech_byGroup_bySeq(filename,featureNum):
...
for i, item in enumerate(h36m_files):
    print('Processing %i of %i (%s)' % (i, len(h36m_files), item))
    clips, speech = process_file_withSpeech_byGroup_bySeq(item, featureNum)

    clips = np.array(clips)     
    clips = np.squeeze(clips) #(3, frames, featureDim:5)

    speech = np.array(speech)    #(3, frames)
    speech = np.squeeze(speech) #(3, frames, featureDim:5)
...
#print("Data shape: {}".format(seq_data[0]).shape))
#np.savez_compressed('data_hagglingSellers_speech_face_60frm_5gap_white_training', clips=data_clips, classes=data_speech)
#np.savez('data_hagglingSellers_speech_group_face_5dim_bySequence_testing', data=seq_data, speech=seq_speech)

output = open('data_hagglingSellers_speech_group_face_5dim_bySequence_training_tiny.pkl', 'wb')
pickle.dump({'data':seq_data, 'speech':seq_speech, 'seqNames': h36m_files}, output)
output.close()
