...
def process_file_withSpeech_face_byGroup(filename, window=240, window_step=120):
    ...
    for j in range(0, len(positions)-window//8, window_step):
        ...
        for pIdx in range(len(positions_list)):
            ...
            faceParamData = faceParamData_group[pIdx]['face_exp']  #200 x frames
...
def get_files_haggling_buyer_body_face(directory, bReturnTesting):
    fileListInit =  [os.path.join(directory,f) for f in sorted(list(os.listdir(directory)))
    if os.path.isfile(os.path.join(directory,f))
    ...
    for f in fileListInit:
        ...
        bInWhiteList = False
        for keyword in white_list_body:  #checking both body and face

            if keyword in f and keyword in white_list_face:     ##Checking both body and face
            #if keyword in f
                bInWhiteList = True
                break
...
h36m_files = get_files_haggling_buyer_body_face('panoptic',bTesting)
...
for i, item in enumerate(h36m_files):
    ...
    clips, speech, face, initPos, initRot = process_file_withSpeech_face_byGroup(item,120,10)

    for pIdx in range(3):
        seq_body[pIdx] += clips[pIdx]
        seq_speech[pIdx] += speech[pIdx]
        seq_face[pIdx] += face[pIdx]
...
for pIdx in range(3):
    seq_body[pIdx] = np.array(seq_body[pIdx])       #(chunkNum, frames, featureDim:73)
    seq_speech[pIdx] = np.array(seq_speech[pIdx])   #(chunkNum, frames)
    seq_face[pIdx] = np.array(seq_face[pIdx])   #(chunkNum, frames)
...
seq_body  = np.array(seq_body)  #(3, chunkNum, frames, featureDim:73)
seq_speech  = np.array(seq_speech)  #(3, chunkNum, frames)
seq_face  = np.array(seq_face)  #(3, chunkNum, frames)
...
print("size: {}".format(seq_body[0].shape))


"""Save as npz"""
np.savez('data_hagglingSellers_speech_body_face_group_120frm_10gap_whiteBF_noGa_training', body=seq_body, face=seq_face, speech=seq_speech, seqNames=h36m_files)#,initPos=seq_initPos, initRot=seq_initRot)
