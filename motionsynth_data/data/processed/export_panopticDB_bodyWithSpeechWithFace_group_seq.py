...
"""
    ...
    windows and windows_speech should have the same size
"""
def process_file_withSpeech_face_byGroup_bySeq(filename, apply_brl=False):
    ...
    face_fileName = seqName[:-7] + '.pkl'
    faceParamPath = faceParamDir + face_fileName
    faceParamData = pickle.load( open(faceParamPath, "rb" ) )
    ...
    for pIdx in range(0,3):
        ...
        speechData_list.append(speechData) #Save speech info
        positions_list.append(positions)    #Save skeleton info #(frames, jointNum, 3)
        ...
        if len(positions) < faceParam.shape[0]:
            faceParam = faceParam[:len(positions),:5]
        else:
            faceParam = faceParam[:,:5]
        faceParam_list.append(faceParam)
    ...
    if apply_brl:       #New ordering: 0:buyer 1:right 2:left  (w.r.t buyer)
        if motionData['rightSellerId'] != motionData['winnerId']:
            ...
            faceParam_list[2], faceParam_list[1] = faceParam_list[1], faceParam_list[2]
    ...
    if len(faceParam_list[0]) != len(faceParam_list[1]): raise Exception()
    if len(faceParam_list[1]) != len(faceParam_list[2]): raise Exception()
        
        
    return positions_list, speechData_list, initInfo_list, faceParam_list
...
for i, item in enumerate(h36m_files):
    ...
    clips, speech, initPos, faceParam = process_file_withSpeech_face_byGroup_bySeq(item, apply_brl=True)

    clips = np.array(clips)     #(3, frames, featureDim:9)
    speech= np.array(speech)    #(3, frames)
    initPos= np.array(initPos)
    faceParam = np.array(faceParam)
...
output = open('data_hagglingSellers_speech_body_face_bySequence_white_noGa_brl_testing_tiny.pkl', 'wb')
pickle.dump({'data':seq_data, 'face': seq_face, 'speech':seq_speech, 'initInfo':seq_initPos, 'seqNames': h36m_files}, output)
output.close()
