...
def process_file_byGroup_perSeq(filename):
...
for i, item in enumerate(h36m_files):
    ...
    clips = process_file_byGroup_perSeq(item)
    clips = np.array(clips)
    basename = os.path.basename(item)

    fileName = outputPath + basename[:-7]
    np.savez(fileName, clips=clips)
