...
def process_file_withSpeech_byGroup(filename, window=240, window_step=120):
    ...
    for j in range(0, frameNum - window, window_step):
        ...
        for pIdx in range(len(motionData_list)):
            ...
            windows_pos_normal[pIdx].append(slice_concat)
...
for i, item in enumerate(h36m_files):
    ...
    clips, speech = process_file_withSpeech_byGroup(item,1,1)
...
np.savez('data_hagglingSellers_speech_formation_1frm_1gap_white_training', clips=group_data, speech=group_speech)
...
#     group_speech[pIdx] = np.array(group_speech[pIdx])
# print("Data shape: {}".format(group_data[0].shape))
# np.savez('data_hagglingSellers_speech_formation_1frm_1gap_white_testing', clips=group_data, speech=group_speech)
...
#     group_speech[pIdx] = np.array(group_speech[pIdx])
# print("Data shape: {}".format(group_data[0].shape))
# np.savez('data_hagglingSellers_speech_formation_30frm_10gap_white_training', clips=group_data, speech=group_speech)
