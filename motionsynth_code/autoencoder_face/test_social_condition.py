...
test_speech_binary =np.min(test_speech_raw,axis=1).astype(np.float32) 
...
preTrainFileName= 'checkpoint_e4900_loss0.0424.pth'


# checkpointFolder = checkpointRoot+ 'social_autoencoder_3conv_vect_vae_conditional/'
# preTrainFileName= 'checkpoint_e1700_loss0.0200.pth'
...
model = modelZoo.autoencoder_3conv_vect_vae_conditional(featureDim, latentDim_vae).cuda()
...
for _, bi in enumerate(batchinds):
    ...
    inputdata_speech = test_speech_binary[idxStart:(idxStart+batch_size)] ##(batch, 1, 120)

    if(inputdata_speech[0,0,0]==0):
        continue
    ...
    for iter in [0,1]:
        #inputdata_speech = 100- inputdata_speech #Take inverse of the label
        inputdata_speech_iter = inputdata_speech*100

        if iter ==1:
            inputdata_speech_iter = 100 -inputdata_speech_iter

        inputData_speech_cuda = Variable(torch.from_numpy(inputdata_speech_iter[:,:,0])).cuda() 
        
        inputData_np_iter = np.concatenate( (inputData_np,inputdata_speech_iter), axis=1)  #(batch, featureDi


        inputData = Variable(torch.from_numpy(inputData_np_iter)).cuda()  #(batch, 73, frameNum)
        ...
        z = model.encode(inputData)
        z = torch.cat( (z,inputData_speech_cuda), 1)
