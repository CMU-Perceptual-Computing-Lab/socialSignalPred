...
class autoencoder_first_16_2layer(nn.Module):
    def __init__(self, featureDim):
        super(autoencoder_first_16_2layer, self).__init__()
        self.encoder = nn.Sequential(
            nn.Dropout(0.25),
            nn.Conv1d(featureDim,16,5,padding=2),        #256, 73, 200
            nn.ReLU(True),
            ...
            nn.Dropout(0.25),
            nn.Conv1d(16,32,5,padding=2),        #256, 73, 200
            nn.ReLU(),
            nn.BatchNorm1d(32),
        ...
        self.decoder2 = nn.Sequential(
            nn.Dropout(0.25),
            nn.ConvTranspose1d(16, featureDim, 5,  padding=2)
            #nn.ReLU(True)
          )  
...
class autoencoder_3conv_vect_vae_conditional(nn.Module):
    def __init__(self,featureDim, latentDim):
        ...
        self.encoder_conv_1 = nn.Sequential(
            #nn.Dropout(0.25),
            nn.Conv1d(self.inputFeat_dim + 1, self.encodeDim_conv1, kernel_size=5, stride=2, padding=2),        #+1 for the label
            nn.ReLU(True),
            nn.BatchNorm1d(self.encodeDim_conv1)
        )
    ...
    def decode(self,z):
        ...
        #z = torch.concat( (z,speechLabel), 1)
