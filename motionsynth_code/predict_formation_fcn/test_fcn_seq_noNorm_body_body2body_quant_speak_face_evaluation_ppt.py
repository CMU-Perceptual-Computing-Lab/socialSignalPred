...
#Own body: for evaluation for thesis
checkpointFolder_body2speak = checkpointRoot+ '/body2speak/social_regressor_fcn_bn_updated2_try1_own_8028/'    
preTrainFileName= 'checkpoint_e311_acc0.8014.pth'
...
log_file_name = os.path.join(checkpointFolder_body2speak, 'opt.json')
import json
with open(log_file_name, 'r') as opt_file:
    options_dict = json.load(opt_file)

    ##Setting Options to Args
    modelName_body2Speak = options_dict['model']

model_body2speak = getattr(modelZoo_body2speak,modelName_body2Speak)().cuda()
#model_body2speak = modelZoo_body2speak.regressor_fcn_bn_dropout().cuda()
#model_body2speak = modelZoo_body2speak.regressor_fcn_bn_updated2().cuda()
...
for seqIdx in range(len(test_X_raw_all)):
    ...
    tjbody_out = np.squeeze(tjbody_out)
    pred_traj2body = tjbody_out
    ...
    if False:
        ...
        eval_in_body_std = (eval_in_body - ae_preprocess['Xmean']) / ae_preprocess['Xstd']
        inputData_ = Variable(torch.from_numpy(eval_in_body_std)).cuda()  #(batch, 3, frameNum)
        eval_output = model_ae_body(inputData_)
        #criterion = nn.MSELoss()
        #criterion(output, inputData_)
        eval_output = eval_output.data.cpu().numpy()  #(batch, 73, frames)
        eval_output = (eval_output * ae_preprocess['Xstd']) + ae_preprocess['Xmean']
        loss_pred = np.mean((eval_output - eval_in_body)**2)#


        eval_in_body = vis_t2body_bodyData[1].copy()
        eval_in_body = eval_in_body[np.newaxis,:,:]
        eval_in_body_std = (eval_in_body - ae_preprocess['Xmean']) / ae_preprocess['Xstd']
        inputData_ = Variable(torch.from_numpy(eval_in_body_std)).cuda()  #(batch, 3, frameNum)
        eval_output = model_ae_body(inputData_)

        eval_in_body = eval_in_body[:,:,:eval_output.shape[2]]
        # criterion = nn.MSELoss()
        # loss_gt = criterion(output, inputData_)
        eval_output = eval_output.data.cpu().numpy()  #(batch, 73, frames)
        eval_output = (eval_output * ae_preprocess['Xstd']) + ae_preprocess['Xmean']
        loss_gt = np.mean((eval_output-eval_in_body)**2)#

        print("{0}".format(loss_pred))
    ...
    if False:
        ...
        eval_in_body = eval_in_body[np.newaxis,:,:]
        #print(eval_in_body[0,10,:100])
        # Data pre-processing
        eval_in_body_std = (eval_in_body - b2s_body_mean) / b2s_body_std

        inputData_ = Variable(torch.from_numpy(eval_in_body_std)).cuda()  #(batch, 3, frameNum)
        output = model_body2speak(inputData_)
        b2speak_pred_speak = output.data.cpu().numpy()  #(batch, 73, frames)  
        b2speak_pred_speak_bin = np.squeeze(b2speak_pred_speak)>0.5
        b2speak_pred_speak_bin = b2speak_pred_speak_bin*1     #0 or 1 value     (frames,)
        vis_b2speak_speak[0] = b2speak_pred_speak_bin
        b2speak_pred_speak =np.squeeze(b2speak_pred_speak) #Overwrite
        
        from sklearn.metrics import accuracy_score
        acc = accuracy_score(vis_b2speak_speak[1][:b2speak_pred_speak_bin.shape[0]], b2speak_pred_speak_bin)
