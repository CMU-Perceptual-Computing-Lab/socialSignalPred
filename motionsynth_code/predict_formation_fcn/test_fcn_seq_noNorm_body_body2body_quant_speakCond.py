...
for seqIdx in range(len(test_X_raw_all)):
    ...
    for iteration in [1]:#[0,1]:  
        ...
        #Speak Prediction
        speak_output_np = np.ones((input_body.shape[0], input_body.shape[1], 1))


        ######################################
        # Data pre-processing
        input_body = np.swapaxes(input_body, 1, 2).astype(np.float32)   #(1, features, frames)
        input_speak = np.swapaxes(speak_output_np, 1, 2).astype(np.float32)   #(1, features, frames)
        ...
        output = model_ae_body.decoder_label(output,input_speak)
