...
train_dblist = ['data_hagglingSellers_speech_body_240frm_5gap_white_training']
test_dblist = ['data_hagglingSellers_speech_body_240frm_10gap_white_testing']


######################################
# Divide data into front and back part 
...
train_X_raw_next = train_X_raw[:,120:,:]
train_speech_raw_next = train_speech_raw[:,120:]
train_X_raw = train_X_raw[:,:120,:]
train_speech_raw = train_speech_raw[:,:120]
...
test_X_raw_next = test_X_raw[:,120:,:]
test_speech_raw_next = test_speech_raw[:,120:]
test_X_raw = test_X_raw[:,:120,:]
test_speech_raw = test_speech_raw[:,:120]
...
train_X_raw_next = train_X_raw_next[speak_time,:,:]
...
train_X_next = np.swapaxes(train_X_next, 1, 2).astype(np.float32) #(num, 73, frameNum)
...
test_X_next = np.swapaxes(test_X_next, 1, 2).astype(np.float32) #(num, 73, frameNum)
#test_Y = np.swapaxes(test_Y, 1, 2).astype(np.float32) #(num, 73, frameNum)

data_all = train_X#np.concatenate( (train_X, train_X), axis= 0)
...
train_X_next = (train_X_next - Xmean) / Xstd
#train_Y = (train_Y - Xmean) / Xstd

test_X = (test_X - Xmean) / Xstd
test_X_next = (test_X_next - Xmean) / Xstd
...
for epoch in range(num_epochs):
    ...
    for bii, bi in enumerate(batchinds):
        ...
        inputData_next_np = train_X_next[idxStart:(idxStart+batch_size),:,:]  #(batch, 73, frameNum)
        ...
        inputData_next = Variable(torch.from_numpy(inputData_next_np)).cuda()  #(batch, 73, frameNum)
        ...
        # ===================forward=====================
        output, mu, logvar, latent = model(inputData)
        loss, recon_loss, kld_loss = modelZoo.vae_loss_function(output, inputData, mu, logvar,criterion,args.weight_kld)

        # #Second data
        output_next, mu_next, logvar_next, latent_next = model(inputData_next)
        loss_next, recon_loss_next, kld_loss_next = modelZoo.vae_loss_function(output, inputData, mu, logvar,criterion,args.weight_kld)

        #Smoothness data
        #smooth_loss = criterion_smooth(latent, latent_next) #Distance in the latent space
        smooth_loss = (( latent - latent_next) **2).mean()

        loss_all = loss + loss_next + args.weight_latentSmooth * smooth_loss
        ...
        avgLoss += (loss.item() + loss_next.item() ) * batch_size *0.5
        avgReconLoss += (recon_loss.item() + recon_loss_next.item()) *batch_size *0.5
        avgKLDLoss += (kld_loss.item()  + kld_loss_next.item()) *batch_size * 0.5
