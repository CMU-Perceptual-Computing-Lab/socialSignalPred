...
# class naive_temporal_baseline(nn.Module):
#     def __init__(self):
#         super(naive_temporal_baseline, self).__init__()
#         self.fc1 = nn.Linear(73*240, 73*120)
#         self.relu1 = nn.Sequential(nn.ReLU(), 
#                         nn.BatchNorm1d(73*120))
#         #self.dout = nn.Dropout(0.2)

#         self.fc2 = nn.Linear(73*120, 73)
#         self.relu2 = nn.Sequential(nn.ReLU(), 
#                         nn.BatchNorm1d(73))
#         #self.dout = nn.Dropout(0.2)

#         self.fc3 = nn.Sequential( nn.Linear(73, 1), 
#                         nn.BatchNorm1d(1)) 
#         self.out_act = nn.Sigmoid()

#     def forward(self, input_):
#         input_ = input_.view(-1,73*240)
#         a1 = self.fc1(input_)
#         h1 = self.relu1(a1)
#         #dout1 = self.dout(h1)
...
#         y = self.out_act(a3)
#         return y
# model = naive_temporal_baseline().cuda()
...
#     def forward(self, x):
#         x = self.encoder_conv(x) #batch, 256, 120
#         x = x.view(-1,256*30)  #batch, 256*120
...
for epoch in range(num_epochs):
    ...
    for bii, bi in enumerate(batchinds):
        ...
        inputDataAll = train_X[idxStart:(idxStart+batch_size),:,0]      
        outputDataAll = train_Y[idxStart:(idxStart+batch_size),0]      
        outputDataAll = outputDataAll[:,np.newaxis]
    ...
    for bii, bi in enumerate(batchinds):
        ...
        inputDataAll = test_X[idxStart:(idxStart+batch_size_test),:,0]      
        outputDataAll = test_Y[idxStart:(idxStart+batch_size_test),0]      
        outputDataAll = outputDataAll[:,np.newaxis]
